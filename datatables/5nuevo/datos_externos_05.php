...
for ($i = 0; $i < $numeroDeColumnas; $i++) {
    if(isset($datosDeLlamada['columns'][$i]['searchable']) && $datosDeLlamada['columns'][$i]['searchable'] == 'true'){
        if(isset($datosDeLlamada['columns'][$i]['search']['value']) && $datosDeLlamada['columns'][$i]['search']['value'] !== ""){
            $reglasDeFiltradoDeUsuario[] = $columnasParaRetorno[$i]." LIKE '%".addslashes($datosDeLlamada['columns'][$i]['search']['value'])."%'";
        }
    }
}
